...
def reset_memory_state() -> None:
    module._current_memory_chance = _MEMORY_REMINDER_CHANCE


class TestMemoryChanceFunctions:
    def test_reset_sets_to_zero(self) -> None:
        _reset_memory_chance()
        assert module._current_memory_chance == 0.0

    def test_step_increments_by_fraction(self) -> None:
        _reset_memory_chance()
        _step_memory_chance()
        expected = _MEMORY_REMINDER_CHANCE / module._MEMORY_COOLDOWN_STEPS
        assert module._current_memory_chance == pytest.approx(expected)

    def test_step_caps_at_max(self) -> None:
        module._current_memory_chance = _MEMORY_REMINDER_CHANCE
        _step_memory_chance()
        assert module._current_memory_chance == pytest.approx(_MEMORY_REMINDER_CHANCE)
...
class TestMemoryToolResetsChance:
    def test_memory_mcp_tool_resets_chance(self) -> None:
        for tool_name in _MEMORY_TOOL_NAMES:
            ...
            _run(hook)
            assert module._current_memory_chance == 0.0, (
                f"Expected reset for {tool_name}"
            )
...
class TestHandlePostToolUse:
    ...
    def test_build_failed_triggers_alert(self) -> None:
        module._current_memory_chance = 0.0
        hook = _make_hook(
            "execute_command", result="BUILD FAILED: something went wrong"
        )
        with patch(
            "cline_hooks.handlers.post_tool_use.random.random", return_value=1.0
        ):
            result = _run(hook)
    ...
    def test_memory_reminder_fires_when_chance_is_max(self) -> None:
        ...
        with patch(
            "cline_hooks.handlers.post_tool_use.random.random", return_value=0.0
        ):
            result = _run(hook)
        assert result is not None
        context = cast(str, result.get("contextModification", ""))
    ...
    def test_step_called_for_file_editing_tools(self) -> None:
        ...
        with patch(
            ...
            _run(hook)
        assert module._current_memory_chance == pytest.approx(
            _MEMORY_REMINDER_CHANCE / module._MEMORY_COOLDOWN_STEPS
        )
