...
from cline_hooks.install import _HOOKS, install
...
class TestInstall:
    ...
    def test_prefers_existing_windows_binary(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("cline_hooks.install._is_windows", return_value=True),
        ):
    ...
    def test_creates_target_directory(self, tmp_path: Path) -> None:
        ...
        with (
            ...
            patch("cline_hooks.install._is_windows", return_value=False),
        ):
            install(str(target))
