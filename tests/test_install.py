...
class TestInstall:
    ...
    def test_prefers_existing_windows_binary(self, tmp_path: Path) -> None:
        ...
        with patch(
            "cline_hooks.install.sys.executable", str(scripts_dir / "python.exe")
        ):
            with patch("cline_hooks.install._is_windows", return_value=True):
    ...
    def test_creates_target_directory(self, tmp_path: Path) -> None:
        ...
        with patch("cline_hooks.install.sys.executable", self._FAKE_PYTHON):
            with patch("cline_hooks.install._is_windows", return_value=False):
                install(str(target))
    ...
    def test_skips_already_correct_symlinks(self, tmp_path: Path) -> None:
        ...
        with patch("cline_hooks.install.sys.executable", self._FAKE_PYTHON):
            with patch("cline_hooks.install._is_windows", return_value=False):
                install(str(tmp_path))
        assert self._normalize_link_target(
            os.readlink(str(tmp_path / _HOOKS[0]))
        ) == self._normalize_link_target(binary)
    ...
    def test_windows_writes_ps1_files_for_all_hooks(self, tmp_path: Path) -> None:
        with patch("cline_hooks.install.sys.executable", "C:/fake/Scripts/python.exe"):
            with patch("cline_hooks.install._is_windows", return_value=True):
                install(str(tmp_path))
